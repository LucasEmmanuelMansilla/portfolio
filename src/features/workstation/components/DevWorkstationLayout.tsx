...
const VSCodeWindow = dynamic(
  () =>
    import("@/src/features/vscode/components/VSCodeWindow").then(
      (mod) => mod.VSCodeWindow
    ),
  { ssr: false }
);
...
export function DevWorkstationLayout({
  ...
  const isVSCodeOpen = useVSCodeWindowStore((store) => store.isOpen);
  const isVSCodeMinimized = useVSCodeWindowStore((store) => store.isMinimized);

  return (
    <div className="relative min-h-dvh w-full overflow-hidden">
      <DesktopBackdrop />

      <div className="pointer-events-none absolute inset-0 z-10 flex pt-10 pb-10 pl-3 pr-3 sm:pt-12 sm:pb-12 sm:pl-4 sm:pr-4 lg:pt-14 lg:pb-14 lg:pl-6 lg:pr-8 xl:pr-10 [@media(max-height:800px)]:pt-8 [@media(max-height:800px)]:pb-8">
        <div className="mt-3 grid h-full min-h-0 w-full grid-cols-[2fr_3fr] gap-4 sm:mt-4 lg:mt-6">
          <div className="flex h-100 w-200 min-h-0 min-w-0 flex-col pt-1 sm:pt-2">
            {showTerminal && (
              <m.div
                variants={fadeVariants}
                initial="initial"
                animate="animate"
                className="pointer-events-auto flex h-full min-h-0 flex-1"
              >
          ...
          <div className="flex min-h-0 min-w-0 flex-col items-end justify-start overflow-hidden pt-1 pr-1 sm:pt-2 sm:pr-2 lg:pr-4 xl:pr-8">
            {showSimulator && (
              <m.div
                ...
                className="pointer-events-auto flex h-full min-h-0 w-full flex-1 justify-end overflow-hidden"
              >
                {simulatorContent}
              </m.div>
            )}
      ...
      {isVSCodeOpen && !isVSCodeMinimized && <VSCodeWindow />}
    </div>
  );
}
